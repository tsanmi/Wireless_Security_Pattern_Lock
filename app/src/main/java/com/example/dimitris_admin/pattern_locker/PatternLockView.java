...
//kuria klasi i opoia dimiourgei to pattern lock stin othoni


import com.example.dimitris_admin.pattern_locker.listener.PatternLockViewListener;
...
import com.example.dimitris_admin.pattern_locker.utils.ResourceUtils;
...
import static com.example.dimitris_admin.pattern_locker.PatternLockView.AspectRatio.ASPECT_RATIO_HEIGHT_BIAS;
import static com.example.dimitris_admin.pattern_locker.PatternLockView.AspectRatio.ASPECT_RATIO_SQUARE;
import static com.example.dimitris_admin.pattern_locker.PatternLockView.AspectRatio.ASPECT_RATIO_WIDTH_BIAS;
import static com.example.dimitris_admin.pattern_locker.PatternLockView.PatternViewMode.AUTO_DRAW;
import static com.example.dimitris_admin.pattern_locker.PatternLockView.PatternViewMode.CORRECT;
import static com.example.dimitris_admin.pattern_locker.PatternLockView.PatternViewMode.WRONG;
...
public class PatternLockView extends View {
    ...
    public  ArrayList<Float> Xpoints=new ArrayList<>();
    public ArrayList<Float> Ypoints=new ArrayList<>();
    public  ArrayList<Float> temp_x_points=new ArrayList<Float>();
    public ArrayList<Float> temp_y_points=new ArrayList<Float>();

    public  ArrayList<Float> presure=new ArrayList<Float>();
    ...
    private void handleActionMove(MotionEvent event) {
        ...
        //to idio ginetai kai edw
        this.Xpoints.add(event.getRawX());
        this.Ypoints.add(event.getRawY());

        this.presure.add(event.getPressure());
    ...
    private void handleActionUp(MotionEvent event) {
        ...
        this.end=SystemClock.elapsedRealtimeNanos();
    ...
    private void handleActionDown(MotionEvent event) {
        ...
        resetPattern();
        //sto Action Down
        //pairnoume tis times pou xreiazomaste
        //kai tis bazoume stis antisoixes arraylist
        float x = event.getX();
        float y = event.getY();
        this.Xpoints.add(event.getRawX());
        this.Ypoints.add(event.getRawY());
    ...
    //xrisimopoiwntas ton idio kwdika pou xrisimopoiei
    //gia na zwgrafizei tis koukides
    //ftiaxnoume ena hashmap to opoio periexei
    //san onoma ton arithmo (1,2,3,...9) kai san kleidi
    //mia arraylist pou periexei ta kentrika x,y
